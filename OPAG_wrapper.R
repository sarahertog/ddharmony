...
OPAG_wrapper <- function(Pop, 
  ...
  } else { # if pop in maxage group is less than 5, then extend to 100 with zeros
    
    age_redist <- maxage
    cv_out <- NA
    pop_ext <- c(Pop5, rep(0, (OAnew - maxage) / 5))
  ...
  pop_ext <- DemoTools::graduate_mono(pop_ext, AgeInt = rep(5,(OAnew + 5)/5), Age= seq(0,OAnew,5), OAG = TRUE)
  Age     <- 0:OAnew
  }
