...
source("Shel/functions/pop_fiveyear_load.R")
source("Shel/functions/estimates_fiveyear_func.R")
...
if(!exists("births_vr")){
  ...
  if(exists("births_vr") & any(class(births_vr) %in% "data.frame")){ 
    df_vr <-estimates_fiveyear_func(births_vr)
    df_vr$process <- "vr"  
  }else
    df_vr <- data.frame() 
...
if(!exists("births_census")){
  ...
  if(exists("births_census") & any(class(births_census) %in% "data.frame")){ 
    df_census <- estimates_fiveyear_func(births_census)
...
assign("fiveyear_final_df", final_df , envir = .GlobalEnv)
...
assign("fiveyear_estimates_df", asbr_df , envir = .GlobalEnv)
}
